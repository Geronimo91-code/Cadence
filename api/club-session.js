...
export default async function handler(req, res) {
  ...
  const clubName = clubDoc.exists ? clubDoc.data().name : '';
  ...
  const session = {
    ...
    durationMin: Number(durationMin) || 90, intent: String(intent).slice(0, 200) || 'Club session set by your coach.', clubId, clubName,
  ...
  for (const m of members.docs) {
    ...
    const doubles = (userSnap.data()?.profile?.doubles || 'no') !== 'no';
    for (const id of ids) {
      const ref = db().doc(`users/${uid}/plans/${id}`);
      const snap = await ref.get();
      if (!snap.exists) continue;
      const plan = snap.data();
      // Replace this club's own earlier team session on that day; other clubs' team sessions stay
      const others = (plan.sessions || []).filter((s) => !(s.day === day && (s.type === 'rest' || (s.teamSession && s.clubId === clubId))));
      const cap = doubles ? 2 : 1;
      // Make room by dropping the athlete's own non-fixed work first
      while (others.filter((s) => s.day === day).length >= cap) {
        const onDay = others.filter((s) => s.day === day);
        const victim = onDay.filter((s) => !s.fixed).pop() || onDay.pop();
        if (!victim) break;
