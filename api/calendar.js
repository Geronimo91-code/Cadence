...
export default async function handler(req, res) {
  ...
  // A second session on the same day is placed by its timeOfDay so the two don't overlap
  const startFor = (s) => {
    if (s.timeOfDay === 'morning') return [7, 30];
    if (s.timeOfDay === 'afternoon') return [14, 0];
    if (s.timeOfDay === 'evening' || s.slot) return [Math.max(hh, 18), mm];
    return [hh, mm];
  };
  ...
  const fmtLocal = (d, h, m) => `${d.getUTCFullYear()}${pad(d.getUTCMonth() + 1)}${pad(d.getUTCDate())}T${pad(h)}${pad(m)}00`;
  ...
  for (const plan of plans) {
    ...
    for (const s of plan.sessions) {
      ...
      const [sh, sm] = startFor(s);
      const start = fmtLocal(day, sh, sm);
      const endD = new Date(Date.UTC(day.getUTCFullYear(), day.getUTCMonth(), day.getUTCDate(), sh, sm + (s.durationMin || 60)));
      ...
      lines.push('BEGIN:VEVENT', `UID:cadence-${u}-${plan.weekId}-${s.day}-${s.slot || 0}`, `DTSTAMP:${new Date().toISOString().replace(/[-:]/g, '').slice(0, 15)}Z`,
